...
data_loader.Make_Examples(
    train_examples_de_en, examples_num=10, shuffle=True, buffer_size=10
)
print("----------------------------------------------------------------------------")
train_examples_en_uk = data_loader.data_load("en", "uk")
data_loader.Make_Examples(
    train_examples_en_uk, examples_num=10, shuffle=True, buffer_size=10
)
print("----------------------------------------------------------------------------")
train_examples_de_ru = data_loader.data_load("de", "ru", "aug", aug=True)
data_loader.Make_Examples(
    train_examples_de_ru, examples_num=10, shuffle=True, buffer_size=10
)
print("----------------------------------------------------------------------------")
train_examples = train_examples_de_en.concatenate(train_examples_en_uk)
train_examples = train_examples.concatenate(train_examples_de_ru)
...
data_loader.Make_Examples(
    train_examples, examples_num=10, shuffle=True, buffer_size=BUFFER_SIZE
)
...
for epoch in range(EPOCHS):
    ...
    if (epoch + 1) % 5 == 0:
        transformer.save("keras_model_trained_" + export_dir + "_" + str(epoch + 1))
        transformer.save_weights(
            "{0}_weights_{1}/{2}_weights".format(export_dir, str(epoch + 1), export_dir)
        )
